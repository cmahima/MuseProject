...
ida_em=clf.fit_transform(X[1:300],y[1:300])
classes = ['Red','Blue','Green']
from matplotlib.colors import ListedColormap
colours = ListedColormap(['r','b','g'])
'''
scatter=plt.scatter(ida_em[:,0],ida_em[:,1],c=y[1:300].ravel(),alpha=0.5, cmap=colours)
plt.xlabel('LDA Component 1', fontsize=15)
plt.ylabel('LDA Component 2', fontsize=15)
plt.legend(handles=scatter.legend_elements()[0], labels=classes,prop={"size":18})
plt.show()
#X=ida_em
'''
...
'''
folds=5
features=['2','1','4','17','18','39','6','5','3','15']
for i in range(1):
    ...
    aucdt=cross_val_score(dt, X, y, cv=cv,scoring='roc_auc_ovr')
    ...
    aucnn=cross_val_score(mlp, X, y, cv=cv,scoring='roc_auc_ovr')
    print("NN  AUC",aucnn)
    cvmlp=scores
    ...
    #mlp.fit(X, y)
    #df1=df.loc[1200:1210,'2':'15']
    #y1=df.loc[1200:1210,'87']
    #print(y1)
    #df1 = df1.astype('float')
    #result=mlp.predict(df1)
    #print(df1)
    ...
    aucrf=cross_val_score(clf, X, y, cv=cv,scoring='roc_auc_ovr')
    print("RF  AUC",aucrf)
    cvrf=scores
    ...
    pre=cross_val_score(svclassifier, X, y, cv=cv,scoring='precision_micro')
    f1=cross_val_score(svclassifier, X, y, cv=cv,scoring='f1_micro')
    #aucrbf=cross_val_score(svclassifier, X, y, cv=cv,scoring='roc_auc_ovo')
    #print("SVM RBF AUC",aucrbf)
    ...
    pre=cross_val_score(clf, X, y, cv=cv,scoring='precision_micro')
    f1=cross_val_score(clf, X, y, cv=cv,scoring='f1_micro')
    print("ADB recall",rec)
    print("ADB Precision",pre)
    print("ADB F1",f1)
    print('Ada Boost Accuracy', scores)
    cvab=scores
    nested_score = cross_val_score(clf, X=X, y=y, cv=cv, scoring=make_scorer(matthews_corrcoef))
    print('matthews_corrcoef',nested_score)
