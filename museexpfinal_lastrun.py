...
expName = 'museexpfinal'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/mahima/Desktop/research/museexpfinal_lastrun.py',
...
welcomeComponents = [welcomescreen]
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if welcomescreen.status == STARTED:
        ...
        if tThisFlipGlobal > welcomescreen.tStartRefresh + 2-frameTolerance:
            # keep track of stop time/frame for later
            welcomescreen.tStop = t  # not accounting for scr refresh
            welcomescreen.frameNStop = frameN  # exact frame index
            win.timeOnFlip(welcomescreen, 'tStopRefresh')  # time at next scr refresh
...
for thisTrial in trials:
    ...
    routineTimer.add(2.000000)
    # update component parameters for each repeat
    images.setImage(imagename)
    # keep track of which components have finished
    Start_trialComponents = [images]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *images* updates
        if images.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
        ...
        if images.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > images.tStartRefresh + 2-frameTolerance:
                # keep track of stop time/frame for later
                images.tStop = t  # not accounting for scr refresh
                images.frameNStop = frameN  # exact frame index
                win.timeOnFlip(images, 'tStopRefresh')  # time at next scr refresh
    ...
    for thisComponent in Start_trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('images.started', images.tStartRefresh)
    trials.addData('images.stopped', images.tStopRefresh)
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    blank_2Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        tThisFlip = win.getFutureFlipTime(clock=blank_2Clock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
...
# update component parameters for each repeat
# keep track of which components have finished
blank_3Components = [text_3, text_2]
